...
export * from "./App/handleFileLoad";
export * from "./App/handleInGameEvent";
export * from "./App/handleInputCode";
export * from "./App/init";
export * from "./App/isTester";
export * from "./App/loadMods";
export * from "./App/onDraw";
export * from "./App/onDrawEvent";
export * from "./App/onFrameUpdate";
export * from "./App/onFrameUpdate";
export * from "./App/preloadImages";
export * from "./App/record";
export * from "./App/registerInputUpdates";
...
export const App = {
  ...
  userId: "_",
  userName: null,
  ENV: location.port == 5500 ? "dev" : "prod",
  sessionId: Math.round(Math.random() * 9999999999),
  playTime: 0,
  gameEventsHistory: {},
  deferredInstallPrompt: null,
  shellBackground: "",
  isOnItch: false,
  isOnElectronClient: false,
  hour: 12,
  misc: {},
  mods: [],
  records: {},
  ...
  applySky() {
    ...
    App.sky.setImage(
      App.preloadedResources[`resources/img/background/sky/${sky}.png`]
    );
    App.skyOverlay.setImage(
      App.preloadedResources[`resources/img/background/sky/${sky}_overlay.png`]
    );
  ...
  setWeather(type) {
    switch (type) {
      case "rain":
        App.skyWeather.image =
          App.preloadedResources["resources/img/background/sky/rain_01.png"];
        App.skyWeather.composite = "xor";
        break;
      case "snow":
        App.skyWeather.image =
          App.preloadedResources["resources/img/background/sky/snow_01.png"];
  ...
  getPetDefFromParents: function (parentA, parentB) {
    ...
    newPetDefinition.friends = [parentA, parentB];
  ...
  createActivePet: function (petDef, props = {}) {
    if (petDef.sprite) {
      App.addEvent(
        `${App.constants.CHAR_UNLOCK_PREFIX}_${PetDefinition.getCharCode(petDef.sprite)}`
      );
    }
  ...
  runRandomEncounters: function () {
    ...
    const shouldDeliver =
      moment().startOf("day").diff(moment(newspaperDeliveryMs), "days") > 0;
  ...
  handlers: {
    show_rating_dialog: function () {
      return App.displayConfirm(
        `If you're enjoying the game, please consider <b>rating it</b> on Itch. Your feedback makes a huge difference and helps us a lot!`,
        [
          {
      ...
      const interval = setInterval(() => {
        if (!App.pet.stats.is_egg) {
          ...
          const tapReminderRemoveHandler = () => {
            ...
            screenWrapper.removeEventListener(
              "click",
              tapReminderRemoveHandler
            );
          };
          ...
          onclick: (username) => {
            ...
            if (!validate(username))
              return App.displayPopup(
                "Username is not valid. Please use A-Z letters and numbers."
              );
            if (username.length < 5)
              return App.displayPopup(
                "Your username cannot have less than 5 characters."
              );
            if (username.length > 18)
              return App.displayPopup(
                "Your username cannot have more than 18 characters."
              );
    ...
    show_newspaper: function (headline, text) {
      ...
      [...container.querySelectorAll(".news-close")].forEach(
        (btn) => (btn.onclick = container.close)
      );
    },
    ...
    open_care_menu: function () {
      const getUnclaimedRewardsBadge = () => {
        return Missions.hasUnclaimedRewards() ? App.getBadge("!") : "";
      };
      App.displayList([
        {
          _mount: (me) => {
        ...
        },
        {
          ...
          onclick: () => {
            App.displayPopup(
              `Tap the screen to pet <b>${App.petDefinition.name}</b><br><br>Don't tap for a few seconds to stop petting`,
              2800,
              () => {
                pet();
              }
        ...
        },
        {
          ...
          onclick: () => {
            if (
              App.hour < App.constants.PARENT_DAYCARE_START ||
              App.hour >= App.constants.PARENT_DAYCARE_END
            ) {
              return App.displayPopup(
                `You can only leave ${App.petDefinition.name} at their parents house between <b>${App.formatTo12Hours(App.constants.PARENT_DAYCARE_START)}</b> and <b>${App.formatTo12Hours(App.constants.PARENT_DAYCARE_END)}</b>`,
                4000
              );
            }

            App.displayConfirm(
              `${App.petDefinition.name} will be with their parents, who will look after them from <b>${App.formatTo12Hours(App.constants.PARENT_DAYCARE_START)}</b> to <b>${App.formatTo12Hours(App.constants.PARENT_DAYCARE_END)}</b>, is that ok?`,
              [
                {
                  name: "yes",
                  onclick: () => {
        ...
        },
        {
          ...
          onclick: () => {
            if (App.petDefinition.lifeStage != 2) {
              return App.displayPopup(
                `${App.petDefinition.name} is not old enough to wear accessories`
              );
            }
    ...
    open_settings: function () {
      const settings = App.displayList([
            ...
            return App.displayList([
              {
                ...
                onclick: () => {
                  ...
                  content.innerHTML = JSON.stringify(
                    App.gameEventsHistory,
                    null,
                    2
                  );
                  App.displayPopup(
                    "array? " + Array.isArray(App.gameEventsHistory)
                  );
                  return true;
                },
              },
              {
                ...
                onclick: () => {
                  ...
                  const content = UI.empty();
                  ui.appendChild(content);
                  content.innerHTML = [...total, `total = ${totalText}`].join(
                    "<br>"
                  );
        ...
        },
        {
          ...
          _mount: (e) =>
            (e.innerHTML = `notifications: <i>${App.settings.notifications ? "on" : "off"}</i>`),
          name: `notifications`,
          onclick: (btn) => {
        ...
        },
        {
          ...
          onclick: () => {
            const display = App.displayList([
              ...
              },
              {
                ...
                onclick: () => {
                  ...
                  if (!App.mods.length)
                    return App.displayPopup("No mods installed");

                  const activeModsList = App.displayList(
                    App.mods.map((modInfo) => {
                      return {
                        name: modInfo.name,
                        onclick: () => {
                          const modInfoScreen = App.displayList([
                            ...
                            },
                            {
                              ...
                              onclick: () => {
                                App.displayConfirm(
                                  `Are you sure you want to uninstall <b>${modInfo.name}</b>?`,
                                  [
                                    {
                                      name: "yes",
                                      onclick: () => {
                                        App.mods.splice(
                                          App.mods.indexOf(modInfo),
                                          1
                                        );
                                        App.sendAnalytics(
                                          "mod_uninstall",
                                        ...
                                        App.displayPopup(
                                          `<b>${modInfo.name}</b> uninstalled successfully, refreshing...`,
                                          null,
                                          () => location.reload()
                                        );
                                      },
            ...
            App.handleFileLoad(
              display.querySelector("#mod-file"),
              "readAsText",
              (data) => {
                try {
                  ...
                  const duplicateIndex = App.mods.findIndex(
                    ({ id }) => id == json.id
                  );
                  if (duplicateIndex != -1) {
                    ...
                    App.displayPopup(
                      `<b>${json.name}</b> updated! <br><br> refreshing...`,
                      2000,
                      () => {
                        location.reload();
                      }
                    ...
                    App.displayPopup(
                      `<b>${json.name}</b> installed! <br><br> refreshing...`,
                      2000,
                      () => {
                        location.reload();
                      }
                  ...
                  console.log(e);
                  App.displayPopup(
                    `Something went wrong, Invalid package: ${e}`
                  );
                }
              }
            );
        ...
        { type: "separator" },
        {
          ...
          onclick: () => {
            return App.displayList([
              {
                _mount: (e) =>
                  (e.innerHTML = `Auto aging: <i>${App.settings.automaticAging ? "On" : "Off"}</i>`),
                onclick: (e) => {
                  if (!App.settings.automaticAging) {
                    App.displayConfirm(
                      `Are you sure? This will make your pets automatically age up after a certain amount of time`,
                      [
                        {
                          name: "yes",
                          onclick: () => {
              ...
              },
              {
                ...
                onclick: (e) => {
                  ...
                  const updateUI = () => {
                    ...
                    const startTime =
                        App.constants.SLEEP_START +
                        App.settings.sleepingHoursOffset,
                      endTime =
                        App.constants.SLEEP_END +
                        App.settings.sleepingHoursOffset;
        ...
        },
        {
          ...
          onclick: () => {
            App.displayList([
              ...
              {
                _mount: (e) =>
                  (e.innerHTML = `classic menu: <i>${App.settings.classicMainMenuUI ? "on" : "off"}</i>`),
                onclick: (item) => {
          ...
          onclick: () => {
            ...
            App.displayList([
              ...
              },
              {
                ...
                onclick: (item) => {
                  App.settings.displayShellButtons =
                    !App.settings.displayShellButtons;
              ...
              {
                _mount: (e) =>
                  (e.innerHTML = `shell shape: <i>${App.settings.shellShape}</i>`),
                onclick: (item) => {
                  App.settings.shellShape++;
                  if (
                    App.settings.shellShape > App.constants.MAX_SHELL_SHAPES
                  ) {
              ...
              },
              {
                _mount: (e) => {
                  const hasNew = Definitions.shell_background.some((entry) => {
                    const isUnlocked =
                      entry.unlockKey && App.getRecord(entry.unlockKey);
              ...
              },
              {
                ...
                onclick: () => {
                  ...
                  App.handleFileLoad(
                    display.querySelector("#shell-image-file"),
                    "readAsDataURL",
                    (data) => {
        ...
        { type: "separator" },
        {
          ...
          onclick: () => {
            ...
            App.displayConfirm(
              `Here you'll be able to copy your unique save code and continue your playthrough on another device`,
              [
                {
                  name: "ok",
                  onclick: () => {
                    App.displayConfirm(
                      `After copying the code, open Tamaweb on another device and paste the code in <b>settings > input code</b>`,
                      [
                        {
                          name: "ok",
                          onclick: () => {
                            try {
                              ...
                              const prompt = App.displayPrompt(
                                `Copy your save code from the box below:<br><small><i class="fa-solid fa-info-circle"></i> starts with <b>save:</b> and ends with <b>:endsave</b></small>`,
                                [
                                  {
                                    name: "Ok, I copied",
                                    class: "back-btn",
        ...
        },
        {
          ...
          onclick: () => {
            App.displayConfirm(
              "Are you sure you want to delete your saved pet?",
              [
                {
                  name: "yes (delete)",
                  onclick: () => {
        ...
        },
        {
          ...
          onclick: () => {
            App.displayConfirm(
              "Are you sure you want to completely delete your data? this will reset your pets, achievements, online id and everything else!",
              [
                {
                  name: "yes",
                  onclick: () => {
                    App.displayConfirm(
                      "Are you sure? There is no way to revert this.",
                      [
                        {
                          name: "yes (delete)",
                          onclick: () => {
        ...
        },
        {
          name: `send feedback`,
          onclick: () => {
            return App.displayPrompt(
              `what would you like to to be added in the next update?`,
              [
                {
                  name: "send",
                  onclick: (data) => {
                    if (!data) return true;
                    App.displayPopup(
                      `<b>Suggestion sent!</b><br> thanks for participating!`,
                      4000
                    );
    ...
    open_stats: function () {
      const list = UI.genericListContainer();
      const content = UI.empty();
      const careRatingIcons = new Array(App.pet.stats.max_care)
        .fill("")
        .map((_, i) => {
          const style =
            i >= App.pet.stats.current_care
              ? "opacity: 0.5; filter:grayscale()"
    ...
    open_character_collection: function () {
      App.addEvent(
        `${App.constants.CHAR_UNLOCK_PREFIX}_${PetDefinition.getCharCode(App.petDefinition.sprite)}`
      );
      ...
      const charactersDef = allCharacters.map((char) => {
        ...
        const isUnlocked = App.getEvent(
          `${App.constants.CHAR_UNLOCK_PREFIX}_${charCode}`
        );
    ...
    open_family_tree: function (petDefinition, usePastTense) {
      ...
      if (!petDefinition.family.length) {
        ...
        if (parents?.length == 2) {
          petDefinition.family = [
            parents.map((parent) => App.minimalizePetDef(parent)),
          ];
        }
      }
      ...
      if (!petDefinition.family.length && !usePastTense) {
        return App.displayPopup(
          `${petDefinition.name} is the pioneer of the family!<br> comeback when your family has grown!`
        );
      }
      ...
      const oldestAncestor = petDefinition.family.length
        ? petDefinition.family[0][0]
        : petDefinition;

      const infoPanelContent = usePastTense
        ? `
    ...
    open_food_list: function (buyMode, activeIndex, filterType) {
        ...
        list.push({
          ...
          onclick: (btn, list) => {
            ...
            if (buyMode) {
              ...
              let nList = App.handlers.open_food_list(
                true,
                sliderInstance?.getCurrentIndex(),
                filterType
              );
            ...
            App.closeAllDisplays();
            let ateFood = App.pet.feed(
              current.sprite,
              current.hunger_replenish,
              current.type,
              null,
              reopenFn
            );
            if (ateFood) {
              ...
              if (App.pet.hasMoodlet("healthy") && food === "medicine")
                App.pet.stats.current_health =
                  App.pet.stats.current_health * 0.6;
              else
      ...
      if (!list.length) {
        App.displayPopup(
          `You don't have any consumables, purchase some from the market`,
          2000
        );
        return;
      }
    ...
    open_feeding_menu: function () {
      App.displayList([
        ...
        },
        {
          ...
          onclick: () => {
            return App.displayConfirm(
              `You take 3 pictures to use as ingredients for your soup! after that, tap to stir until it's mixed!`,
              [
                {
                  name: "start",
                  onclick: () => cookingGame(),
    ...
    open_stats_menu: function () {
      const hasNewlyUnlockedAchievements =
        App.handlers.open_achievements_list(true);
      App.displayList([
        {
          name: "stats",
          onclick: () => {
        ...
        },
        {
          ...
          onclick: () => {
            const generations = App.petDefinition.deceasedPredecessors.map(
              (def) => {
              ...
              [
                {
                  ...
                  onclick: (value) => {
                    ...
                    App.save();
                    App.displayPopup(
                      `Name set to "${App.pet.petDefinition.name}"`
                    );
                  },
                },
                { name: "cancel", class: "back-btn", onclick: () => {} },
    ...
    open_achievements_list: function (checkIfHasNewUnlocks) {
      ...
      const list = Object.keys(Definitions.achievements)
        .map((id) => {
          const { name, description, checkProgress, getReward } =
            Definitions.achievements[id];
        ...
        .sort((a, b) =>
          a.isNewlyUnlocked == b.isNewlyUnlocked
            ? 0
            : a.isNewlyUnlocked
              ? -1
              : 1
    ...
    open_item_list: function (buyMode, activeIndex, customPayload) {
        ...
        list.push({
          ...
          onclick: (btn, list) => {
            if (buyMode) {
              ...
              // console.log(list.scrollTop);
              let nList = App.handlers.open_item_list(
                true,
                sliderInstance?.getCurrentIndex()
              );
              // nList.scrollTop = list.scrollTop;
      ...
      if (!list.length) {
        App.displayPopup(
          `You don't have any items, purchase some from the mall`,
          2000
        );
        return;
      }
    ...
    open_shell_background_list: function () {
      ...
      const list = Definitions.shell_background
        .filter(
          (current) => !(current.unlockKey && !App.getRecord(current.unlockKey))
        )
        .map((current) => {
          // check for unlockables
    ...
    open_accessory_list: function (buyMode, activeIndex, customPayload) {
      ...
      for (let accessoryName of Object.keys(Definitions.accessories)) {
        ...
        list.push({
          ...
          onclick: (btn, list) => {
            ...
            // toggle equip mode
            if (equipped)
              App.petDefinition.accessories.splice(
                App.petDefinition.accessories.indexOf(accessoryName),
                1
              );
            else App.petDefinition.accessories.push(accessoryName);
            getDressed(() => App.pet.createAccessories(), reopen, !equipped);
      ...
      if (!list.length) {
        App.displayPopup(
          `You don't have any accessories, purchase some from the mall`,
          2000
        );
        return;
      }
      ...
      sliderInstance = App.displaySlider(
        ...
        activeIndex,
        {
          accept: buyMode ? "Purchase" : "Toggle",
        },
        buyMode
          ? `$${App.pet.stats.gold + (salesDay ? ` <span class="sales-notice">DISCOUNT DAY!</span>` : "")}`
          : null
    ...
    open_friends_list: function (onClickOverride) {
      if (!App.petDefinition.friends.length) {
        App.displayPopup(
          `${App.petDefinition.name} doesn't have any friends right now<br><br><small>Visit the park to find new friends<small>`,
          4000
        );
        return;
      }

      const friendsList = App.displayList(
        App.petDefinition.friends.map((friendDef, index) => {
          ...
          return {
            ...
            onclick: () => {
              ...
              const friendActivitiesList = App.displayList([
                ...
                {
                  _ignore:
                    App.petDefinition.lifeStage < 2 ||
                    friendDef.lifeStage < 2 ||
                    friendDef.stats.is_player_family,
                  name: `go on date`,
                  onclick: () => {
                    if (friendDef.getFriendship() < 60) {
                      return App.displayPopup(
                        `${App.petDefinition.name}'s friendship with ${friendDef.name} is too low <br><br> they don't want to go on a date.`,
                        5000
                      );
                    }

                    App.displayConfirm(
                      `Do you want to go on a date with <div>${icon} ${friendDef.name}</div>?`,
                      [
                        {
                          name: "yes",
                          onclick: () => {
                ...
                },
                {
                  ...
                  onclick: () => {
                    App.displayConfirm(
                      `Are you sure you want to give gift to ${icon} ${name}?`,
                      [
                        {
                          name: "yes",
                          onclick: () => {
                            ...
                            App.handlers.open_item_list(null, null, (item) => {
                              ...
                              friendDef.increaseFriendship(
                                Math.floor(item.price / 2.7)
                              );
                              inviteGiveGift(friendDef);
                            });
                            return true;
                          },
                        },
                ...
                },
                {
                  ...
                  onclick: () => {
                    App.displayConfirm(
                      `Are you sure you want to unfriend ${icon} ${name}?`,
                      [
                        {
                          name: "yes",
                          onclick: () => {
      ...
      App.displayList([
        {
          ...
          onclick: () => {
            if (App.petDefinition.lifeStage <= 0) {
              return App.displayPopup(
                `${App.petDefinition.name} is not old enough to go to hubchi!`
              );
            }
            ...
            if (App.pet.stats.current_sleep < 10) {
              return App.displayPopup(
                `${App.petDefinition.name} is too sleepy to go to HUBCHI!`
              );
            }
            ...
            return App.displayConfirm(`Enter Hubchi?`, [
              {
                ...
                onclick: async () => {
                  App.closeAllDisplays();
                  onlineHubTransition(async (fadeOverlay) => {
                    setTimeout(() =>
                      App.playSound("resources/sounds/task_complete.ogg", true)
                    );
                    const popup = App.displayPopup("Connecting...", App.INF);
                    App.temp.online = {};
                    App.temp.online.hasUploadedPetDef =
                      await App.apiService.getPetDef();
                    App.temp.online.randomPetDefs =
                      await App.apiService.getRandomPetDefs(7);
        ...
        },
        {
          ...
          onclick: () => {
            ...
            if (moment().utc().isBefore(nextBirthday)) {
              return App.displayPopup(
                `${App.petDefinition.name} hasn't grown enough to age up<br><br>come back <b>${moment(nextBirthday).utc().fromNow()}</b>`,
                5000
              );
            }
            App.displayConfirm(
              `This will age up ${App.petDefinition.name}<br>Are you sure?`,
              [
                {
                  name: "yes",
                  onclick: () => {
        ...
        },
        {
          ...
          onclick: () => {
            ...
            App.displayConfirm(
              `Are you sure you want to send ${App.petDefinition.name} on a vacation? <br> it will cost you $${price} and ${App.petDefinition.name} will stay there until you decide to end their vacation <hr> <b>while on vacation, ${App.petDefinition.name}'s needs will not drop</b>`,
              [
                {
                  name: `yes ($${price})`,
                  onclick: () => {
            ...
            App.displayList([
              {
                ...
                onclick: () => {
                  let charCode =
                    "friend:" + btoa(JSON.stringify(window.localStorage));
                  navigator.clipboard.writeText(charCode);
                    ...
                    [
                      {
                        ...
                        onclick: () => {
                          App.displayConfirm(
                            `Send it to your friend and they'll be able to add ${App.petDefinition.name} as a friend using <b>Phone > Friend Codes > Input code</b>`,
                            [
                              {
                                name: "ok",
                                onclick: () => {},
                  ...
                  App.displayPrompt(`enter your friend code:`, [
                    {
                      ...
                      onclick: (rawCode) => {
                        ...
                        try {
                          ...
                          App.displayConfirm(
                            `Are you trying to add <div style="font-weight: bold">${def.getCSprite()} ${def.name}?</div> as a friend?`,
                            [
                              {
                                name: "yes",
                                onclick: () => {
    ...
    open_social_media: function () {
      ...
      function showPost(petDefinition, noMood, noNextBtn) {
        if (++App.temp.seenSocialMediaPosts >= 12) {
          return App.displayPopup(
            "There are no more social media posts, comeback later!"
          );
        }
        ...
        let post = document
          .querySelector(".cloneables .post-container")
          .cloneNode(true);
        document.querySelector(".screen-wrapper").appendChild(post);
        post.style.display = "";

        document.querySelector(".post-profile-icon").innerHTML =
          `${petDefinition.getCSprite()}`;
        ...
        if (petDefinition !== App.petDefinition)
          homeBackground = randomFromArray(
            Object.keys(Definitions.room_background).map(
              (roomName) => Definitions.room_background[roomName].image
            )
          );
        ...
        const character = new Object2d({
          drawer: postDrawer,
          spritesheet: {
            ...petDefinition.spritesheet,
            cellNumber: randomFromArray(characterSpritePoses),
          },
          // image: App.pet.image.cloneNode(),
          // img: petDefinition.sprite,
          image: App.preloadedResources[petDefinition.sprite],
          x: randomFromArray(characterPositions),
          y: 55 + (petDefinition.spritesheet.offsetY * 2 || 0),
        });
        ...
        },
        {
          ...
          onclick: () => {
            ...
            let potentialFriends = new Array(8)
              .fill(undefined)
              .map((spot, i) =>
                App.getRandomPetDef(App.petDefinition.lifeStage, seed + i * 128)
              );
            App.displayGrid([
              ...potentialFriends.map((otherPetDef) => {
                return {
                  name: `${otherPetDef.getFullCSprite()}`,
                  class: "bg-bef-1",
                  onclick: () => {
                    App.displayConfirm(
                      `Do you want to send friend request to<br><b>${otherPetDef.getCSprite()} ${otherPetDef.name}?</b>`,
                      [
                        {
                          name: "yes",
                          onclick: () => {
                            ...
                            if (!willAcceptFriendRequest) {
                              App.displayPopup(
                                `${otherPetDef.name} did <b style="color: #ff6e74">not accept</b> ${App.petDefinition.name}'s friend request`
                              );
                              return;
                            }
                            let state =
                            ...
                            if (state)
                              App.displayPopup(
                                `${otherPetDef.name} <b style="color: #87cf00">accepted</b> ${App.petDefinition.name}'s friend request!`
                              );
                            else
                              App.displayPopup(
                                `${App.petDefinition.name} is already friends with ${otherPetDef.name}!`
                              );
                          },
                        },
                        {
          ...
          onclick: () => {
            App.handlers.open_friends_list((friendDef) => {
              if (friendDef.sentMessage) {
                App.displayPopup(
                  `${App.petDefinition.name} shouldn't spam ${friendDef.name}'s inbox!`,
                  3000
                );
                return;
              }
    ...
    open_mall_activity_list: function () {
      const hasNewDecor = Object.keys(Definitions.room_background).some(
        (key) => {
          const isUnlocked = Definitions.room_background[key].unlockKey
            ? App.getRecord(Definitions.room_background[key].unlockKey)
            : true;
          return Definitions.room_background[key].isNew && isUnlocked;
        }
      );
      const hasNewAccessory = Object.keys(Definitions.accessories).some(
        (key) => {
          const isUnlocked = Definitions.accessories[key].unlockKey
      ...
      App.displayList([
        ...
        },
        {
          ...
          onclick: () => {
            ...
            if (App.petDefinition.lifeStage != 2) {
              App.displayPopup(
                `${App.petDefinition.name} is not old enough to wear accessories yet, but you can buy some for later`
              );
            }
            return true;
          },
      ...
      App.displayList([
        {
          ...
          onclick: () => {
            ...
            const images = `
                        ...
                        `;
            App.displayPopup(
              `Try to predict your opponents next stance ${images} and mimic them!`,
              tutorialDisplayTime,
              () => opponentMimicGame()
            );
            return false;
        ...
        },
        {
          ...
          onclick: () => {
            App.displayPopup(
              `Catch as much <img src="resources/img/misc/heart_particle_01.png"></img> while avoiding <img src="resources/img/misc/falling_poop.png"></img>`,
              tutorialDisplayTime,
              () => fallingStuffGame()
            );
            return false;
        ...
        },
        {
          ...
          onclick: () => {
            App.displayPopup(
              `Stop the pointer at the perfect time!`,
              tutorialDisplayTime,
              () => barTimingGame()
            );
            return false;
  ...
  toggleGameplayControls: function (state, onclick) {
    ...
    if (App.settings.classicMainMenuUI) {
      if (state)
        document
          .querySelector(".classic-main-menu__container")
  ...
  createProgressbar: function (percent) {
    let progressbar = document
      .querySelector(".cloneables .progressbar")
      .cloneNode(true);

    let rod = progressbar.querySelector(".progressbar-rod"),
      background = progressbar.querySelector(".progressbar-background");
  ...
  displayList: function (listItems, backFn, backFnTitle) {
    ...
    listItems.forEach((item, i) => {
      ...
      switch (item.type) {
        ...
        default:
          ...
          if (item.name.indexOf("<") == -1 && item.name.indexOf("/") == -1)
            item.name = ellipsis(item.name, item.ellipsisLength);
          element.innerHTML = item.name;
      ...
      element.className =
        defaultClassName + (item.class ? " " + item.class : "");
  ...
  displayGrid: function (listItems) {
    ...
    let list = document
      .querySelector(".cloneables .generic-grid-container")
      .cloneNode(true);
  ...
  displaySlider: function (listItems, activeIndex, options, additionalText) {
    let list = document
      .querySelector(".cloneables .generic-slider-container")
      .cloneNode(true);
    ...
    const changeIndex = (diff) => {
      ...
      let animationStart =
        diff < 0 ? "slider-item-anim-in-right" : "slider-item-anim-in-left";
    ...
    list.querySelector(".slide-left").onclick = () => {
      contentElement.querySelector(".slider-item").style.animation =
        "slider-item-anim-out-left 0.1s linear forwards";
      setTimeout(() => changeIndex(-1), 150);
    };
    list.querySelector(".slide-right").onclick = () => {
      contentElement.querySelector(".slider-item").style.animation =
        "slider-item-anim-out-right 0.1s linear forwards";
      setTimeout(() => changeIndex(1), 150);
    };
  ...
  displayPopup: function (content, ms, onEndFn, isReveal) {
    let popup = document
      .querySelector(".cloneables .generic-list-container")
      .cloneNode(true);
  ...
  displayEmpty: function (addClass) {
    let display = document
      .querySelector(".cloneables .generic-empty-container")
      .cloneNode(true);
  ...
  initSound: function () {
    ...
    document.addEventListener("click", (e) => {
      ...
      if (
        ...
        App.vibrate();
        if (
          backSoundClassNames.some((n) => e.target.classList.contains(n)) ||
          e.target.textContent.toLowerCase() == "back"
        )
          this.playSound(`resources/sounds/ui_click_02.ogg`, true);
        else this.playSound(`resources/sounds/ui_click_01.ogg`, true);
      }
      ...
      if (
        ...
        const previousListItem = [
          ...document.querySelectorAll(
            ".screen-wrapper .generic-list-container"
          ),
        ].at(-1);
        if (previousListItem) {
          previousListItem.transitionAnim?.();
          [...previousListItem.children].forEach((child) =>
            child?._mount?.(child)
          );
        }
        UI.lastClickedButton = null;
  ...
  getDayId: function (forCurrentUser) {
    if (forCurrentUser) {
      return +(App.getDayId() + (App.userId || 1234)).slice(0, 16);
    }
  ...
  getFoodCSprite: function (index) {
    ...
    const size =
      FOOD_SPRITESHEET_DIMENSIONS.rows * FOOD_SPRITESHEET_DIMENSIONS.cellSize;
    return `<c-sprite 
            naturalWidth="${size}" 
            naturalHeight="${size}" 
  ...
  playSound: function (path, force) {
    ...
    if (this.audioElement.src != path) this.audioElement.src = path;
  ...
  save: function (noIndicator) {
    ...
    window.localStorage.setItem(
      ...
      JSON.stringify(App.gameEventsHistory)
    );
  ...
  load: function () {
    ...
    let userId =
      ...
      random(100000000000, 999999999999);
    App.userId = userId;
    ...
    let shellBackground =
      ...
      Definitions.shell_background[1].image;
  ...
  setShellBackground: function (url) {
    ...
    document.querySelector(
      "body > div.root > div.dom-shell"
    ).style.backgroundImage = `url(${url})`;
    document.querySelector(".background").style.backgroundImage = `url(${url})`;
    App.shellBackground = url;

    document.querySelector(".shell-btn.main").style.backgroundImage =
      `url(${App.shellBackground})`;
    document.querySelector(".shell-btn.right").style.backgroundImage =
      `url(${App.shellBackground})`;
    document.querySelector(".shell-btn.left").style.backgroundImage =
      `url(${App.shellBackground})`;
    return true;
  },
  ...
  useWebcam: function (callback, facingMode, shutterDelay) {
    ...
    function unloadWebcam() {
      App.toggleGameplayControls(
        gameplayControlsState.state,
        gameplayControlsState.onclick
      );
    ...
    const webcamChangeButton = videoContainer.querySelector(
  ...
  checkPetStats: function () {
    ...
    App.createNotification(
      "this is on testing interval",
      `Hello my name is ${Math.random()}`
    );
    setTimeout(() => {
      App.checkPetStats();
    }, 10000);
  ...
  apiService: {
    ENDPOINT:
      "https://script.google.com/macros/s/AKfycbxCa6Yo_VdK5t9T7ZCHabxT1EY-xACEC3VUDHgkkwGdduF2U5VMGlp0KXBu9CtE8cWv9Q/exec",
    ENDPOINT_TEST:
      "https://script.google.com/macros/s/AKfycbzvoH9j7Ia0Zc_dCBXXYI6dB9UlUR_tGGr1J5Gsu2DG/dev",
    _getUid: () => {
