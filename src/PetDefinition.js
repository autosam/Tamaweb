...
export class PetDefinition {
  ...
  serializeStats(noStringify) {
    ...
    this.serializables.forEach((serializable) => {
      if (serializable === "stats") {
        s["stats"] = {
          // todo: for testing, might want to revert
  ...
  getFriendship() {
    if (!this.stats.player_friendship) this.increaseFriendship(random(2, 8));
  ...
  ageUp(isNpc) {
    ...
    if (!possibleEvolutions) {
      possibleEvolutions =
        GROWTH_CHART[randomFromArray(Object.keys(GROWTH_CHART))];
    }
    ...
    switch (this.lifeStage) {
      case 0:
        ...
        switch (careRating) {
          case 1:
            targetEvolutions = possibleEvolutions.slice(0, 2);
            break; // low care
          case 3:
            targetEvolutions = possibleEvolutions.slice(5);
            break; // high care
          default:
            targetEvolutions = possibleEvolutions.slice(2, 5); // default medium care
        }
      ...
      case 1:
        switch (careRating) {
          case 1:
            this.sprite = possibleEvolutions[0];
            break; // low care
          case 3:
            this.sprite = possibleEvolutions[2];
            break; // high care
          default:
            this.sprite = possibleEvolutions[1]; // default medium care
        }
        break;
      case 2:
        return;
  ...
  addFriend(friendDef, friendship) {
    if (this.friends.indexOf(friendDef) === -1) {
  ...
  refreshWant(currentTry = 1, existingCurrentCategory, forced) {
    ...
    if (current_want.type) {
      // has unfulfilled want
    ...
    const currentCategory =
      /* 'minigame' ||  */ existingCurrentCategory ||
      randomFromArray(possibleCategories);
