...
export class Object2d {
  ...
  setImage(image) {
    // this one gets img object (presume preloadedResource)
    this.imageSrc = image.src;
    this.image = image;
  ...
  moveToTarget(speed = 0.01) {
    if (this.targetX !== undefined && this.targetX != this.x) {
      this.isMoving = true;
      if (this.x > this.targetX) this.moveLeft(this.targetX, speed);
      else if (this.x < this.targetX) this.moveRight(this.targetX, speed);
    } else {
      this.isMoving = false;
    }
    ...
    if (this.targetY !== undefined && this.targetY != this.y) {
      ...
      if (this.y > this.targetY) this.moveUp(this.targetY, speed);
      else if (this.y < this.targetY) this.moveDown(this.targetY, speed);
    }
  }
  ...
  static animations = {
    ...
    pulseScale: function (me, speed, strength) {
      ...
      if (!me.pulseScaleFloat || me.pulseScaleFloat >= App.PI2)
        me.pulseScaleFloat = 0;
