...
export class Pet extends Object2d {
      ...
      onDraw: (overlay) => {
        ...
        if (this.staticShadow) {
          overlay.y =
            this.y +
            this.additionalY +
            Math.ceil(this.spritesheet.cellSize / 2.1);
          return;
        }
  ...
  createAccessories() {
    ...
    this.petDefinition.accessories.forEach((accName) => {
      ...
      const accessoryObject = accessory.createFn
  ...
  handleDead() {
    ...
    App.toggleGameplayControls(false, () => {
      ...
      App.displayConfirm(
        `Do you want to receive a new egg?`,
        [
          {
            name: "yes",
            onclick: () => {
  ...
  useItem(item) {
    ...
    this.triggerScriptedState(
      ...
      Pet.scriptedEventDrivers.playingWithItem.bind({
        pet: App.pet,
        item: item,
        itemObject,
      }),
  ...
  playCheeringAnimationIfTrue(requirement, onEndFn) {
    if (requirement) this.playCheeringAnimation(onEndFn);
    else if (onEndFn) onEndFn();
  }
  ...
  handleRandomGestures() {
    ...
    const hasBadMoodlets = ["bored", "sleepy", "hungry", "sick"]
      .map((moodName) => this.hasMoodlet(moodName))
      .some((moodlet) => moodlet);
  ...
  statsManager(isOfflineProgression, hour) {
    ...
    if (this.stats.is_at_parents) {
      if (
        hour < App.constants.PARENT_DAYCARE_START ||
        hour >= App.constants.PARENT_DAYCARE_END
      ) {
        if (!isOfflineProgression) {
    ...
    let cleanliness_depletion_rate =
      stats.cleanliness_depletion_rate * depletion_mult;
    ...
    if (this.stats.is_sleeping || offlineAndIsNight) {
      ...
      sleep_depletion_rate =
        -stats.sleep_replenish_rate * sleepAdditionalDepletionMult;
    }
    ...
    if (this.stats.has_poop_out || !this.dirtyOverlay.hidden) {
      // gradually decrease health if poop is nearby or dirty
      stats.current_health -=
        health_depletion_rate * stats.health_depletion_mult;
      stats.current_cleanliness -=
        cleanliness_depletion_rate * stats.cleanliness_depletion_mult;
    }
    ...
    careAffectingStats.forEach((statName) => {
      // decreasing
      if (previousStats[statName] != 0 && this.stats[statName] == 0) {
  ...
  stateManager() {
    if (this.scriptedEventTime) {
      if (this.scriptedEventTime < App.lastTime) {
        // ending scripted event time
        if (this.stopScriptedState()) return;
      } else {
        // during scripted event
    ...
    if (this.stats.is_sleeping) {
      if (
        (this.stats.current_sleep >= this.stats.max_sleep ||
          (this.hasMoodlet("rested") &&
            Math.random() < this.stats.light_sleepiness * 0.01)) &&
        !App.isSleepHour()
      ) {
    ...
    } else if (this.isMoving) this.setState("moving");
    else {
      if (this.state.indexOf("idle") == -1) {
        if (
          this.hasMoodlet("hungry") ||
          this.hasMoodlet("sleepy") ||
          this.hasMoodlet("bored") ||
          this.hasMoodlet("sick")
        ) {
          if (random(0, 1)) this.setState("idle_uncomfortable");
          else this.setState("idle_side_uncomfortable");
        } else {
          this.setState("idle");
        }
      }
  ...
  handleAnimation() {
    ...
    if (this.animation.nextFrameTime < App.lastTime) {
      // go to next frame
      ...
      this.animation.currentFrame =
        (this.animation.currentFrame + 1) % frameRound;
  ...
  wander() {
    ...
    if (!this.nextRandomTargetSelect) {
      this.nextRandomTargetSelect =
        App.lastTime +
        random(this.stats.wander_min, this.stats.wander_max) * 1000;
    }
  ...
  getStatsDepletionRates(offline) {
    ...
    for (let i = 0; i < seconds; i++) {
      ...
      if (this.stats.current_hunger <= 0 && !report.hunger)
        report.hunger = { ...min, stat: this.stats.current_hunger };
      if (this.stats.current_sleep <= 2 && !report.sleep)
        report.sleep = { ...min, stat: this.stats.current_sleep };
      if (this.stats.current_fun <= 0 && !report.fun)
        report.fun = { ...min, stat: this.stats.current_fun };
      if (this.stats.current_bladder <= 3 && !report.bladder)
        report.bladder = { ...min, stat: this.stats.current_bladder };
      if (this.stats.current_cleanliness <= 0 && !report.cleanliness)
        report.cleanliness = { ...min, stat: this.stats.current_cleanliness };
      if (this.stats.current_health <= 0 && !report.health)
        report.health = { ...min, stat: this.stats.current_health };
      if (this.stats.current_death_tick <= 0 && !report.death_tick)
        report.death_tick = { ...min, stat: this.stats.current_death_tick };
    }
  ...
  showThought(type, item) {
    const bubble = new Object2d({
      ...
      onDraw: (me) => {
        ...
        me.y =
          this.y -
          this.spritesheet.cellSize * 1.5 -
          (this.spritesheet.offsetY * 1.8 || 0) +
          Math.sin(me.float);
    ...
    switch (type) {
      ...
      case App.constants.WANT_TYPES.fulfilled:
        new Object2d({
          parent: bubble,
          image:
            App.preloadedResources["resources/img/misc/want_fulfilled.png"],
          x: 10,
          y: 10,
          z: 10,
          onDraw: (me) => {
  ...
  static scriptedEventDrivers = {
    playing: function (start) {
      ...
      if (this.pet.x == this.pet.targetX) {
        if (this.pet.targetX == 0)
          this.pet.targetX = 100 - this.pet.spritesheet.cellSize;
